...
def compute_gc_content_ks_test(seq_df):
    loci_gc = seq_df.query("type == 'loci'")["gc_percent"].values
...
def compute_total_counts_stats(seq_df):
    loci_df = seq_df.query("type == 'loci'")
...
def generate_html_report(seq_df, output_dir="report_output", filename="report.html"):
